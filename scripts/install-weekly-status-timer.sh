...
usage() {
  cat <<'USAGE'
Usage: scripts/install-weekly-status-timer.sh [options]

Install a systemd --user timer that runs one public-safe LearnBuddy weekly parent report.
...
Options:
  ...
  --on-calendar SPEC       systemd OnCalendar spec, e.g. 'Sun 19:00' (default: Sun 19:00)
...
The service uses: learnbuddy weekly-status --notify
It respects pause-today, once-per-week, and empty-week guards. Empty weekly reports are skipped by default.
USAGE
}
...
mkdir -p "$UNIT_DIR"
SERVICE_NAME="learnbuddy-weekly-status-${PROFILE}.service"
TIMER_NAME="learnbuddy-weekly-status-${PROFILE}.timer"
...
EXEC_ARGS=("weekly-status" "--notify")
if [[ -n "$CONFIG_PATH" ]]; then
...
{
  echo "[Unit]"
  echo "Description=LearnBuddy weekly parent report (${PROFILE})"
  echo
  echo "[Service]"
  echo "Type=oneshot"
...
[Unit]
Description=Run LearnBuddy weekly parent report (${PROFILE})
...
echo "command=learnbuddy weekly-status --notify"
