...
- material/file parity: `learnbuddy material add-text` and `learnbuddy material add-file` store pasted text, text/PDF files, or OCR-extracted worksheet photos as parent-review state before approval; images require an explicit `LEARNBUDDY_MATERIAL_OCR_COMMAND`/`--ocr-command` and never become child-visible before expected answers are approved
